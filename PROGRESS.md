...
| 2026-09-17 | **commit msg 补充约定：描述里禁止出现"阶段N / 第N步"等进度编号** | 用户要求。commit 只描述"改了什么"，进度追踪归 `PROGRESS.md`，二者职责分离。AI 规范 §5.5、手册 `10` §22 已同步。⚠️ 已推送的 `ea102ff` / `7d7f4fa` 两条含步骤号，**用户未要求重写历史，保持原样** |
| 2026-09-17 | 🛑 **提交权限：任何 `git commit` / `git push` 都必须先经用户明确同意** | 用户要求，**优先级高于"一步一 commit"**。AI 完成一步只汇报「改动 + 编译结果 + 建议 message」，等用户说"提交"才执行；不先 `git add`。写文件不受限制，被拦的只是提交与推送两个动作。AI 规范 §5.5 已同步 |
| 2026-09-17 | 🚫 **代码注释里禁止出现文档引用与进度编号**（书名号文档名、`§`章节号、文件路径、`阶段N`、`第N步`、`详见 XXX`） | 用户要求。注释要自包含地说明技术原因，不把读者引去翻文档；文档改名后引用即失效。AI 规范 §4.5 已同步 |
| 2026-09-17 | **环境方案变更：弃用 Linux 虚拟机，改用 Docker Desktop（WSL2 后端）** | 原 CentOS 7 VM 因 `hwclock` 触发内核 panic、系统时钟落后 RTC 4 个月、sshd 失效、IP 漂移等一连串问题而不可用。Windows 宿主时钟正确、无跨机网络，中间件统一在 `127.0.0.1`。代价：需装 WSL2 + Docker Desktop（已完成） |
