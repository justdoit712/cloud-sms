  ...
  <div class="api-mapping">
    ...
    >
      <el-form
        ...
        label-width="120px"
      >
        <el-form-item label="API 名称" prop="gatewayApiName">
          <el-input v-model="formModel.gatewayApiName" placeholder="请输入 API 网关注册名称" />
        </el-form-item>
        <el-form-item label="映射网关服务" prop="serviceId">
          <el-input v-model="formModel.serviceId" placeholder="目标后台服务名称 (例如: beacon-sms)" />
        </el-form-item>
        <el-form-item label="映射真实地址" prop="insideApiUrl">
          <el-input v-model="formModel.insideApiUrl" placeholder="内部实际请求 URL" />
        </el-form-item>
        <el-form-item label="介绍说明" prop="description">
          <el-input v-model="formModel.description" placeholder="请输入接口简介" />
        </el-form-item>
...
const searchConfig = [
  { label: 'API名称', prop: 'search', type: 'input', placeholder: '请输入网关API名搜索' }
] as any[]

const columns = [
  { type: 'selection', width: 60 },
  { prop: 'id', label: '编号', width: 80, sortable: true },
  { prop: 'gatewayApiName', label: '网关API名称' },
  { prop: 'serviceId', label: '服务网关ID' },
  { prop: 'insideApiUrl', label: '内部映射地址' },
  { prop: 'description', label: '接口描述', showOverflowTooltip: true },
...
const formRules = {
  gatewayApiName: [{ required: true, message: 'API名称不能为空', trigger: 'blur' }],
  serviceId: [{ required: true, message: '映射服务名称不能为空', trigger: 'blur' }],
  insideApiUrl: [{ required: true, message: '映射地址不能为空', trigger: 'blur' }]
}
...
async function handleEdit() {
  ...
  try {
    ...
    ElMessage.error(error.message || '获取映射详情失败')
  }
}
