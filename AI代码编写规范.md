...
> 定位：代码阶段启动后，本文件是**最高优先级、自包含的代码约束**。本文件不依赖任何其它文档即可独立执行；内部文档（`docs/learning/00`、`docs/learning/06`、`docs/02_项目详细分析总报告.md`）只作任务范围与背景参考。
...
7. **禁止行为清单**：
   ...
   - 禁止编造 Nacos 配置项；新增配置必须在 `docs/learning/06` 的对应模块清单中登记
...
- `docs/learning/06_逐模块重构方案.md` —— 9 模块执行蓝图（任务范围以此为准）
- `docs/learning/07_索引与总览.md` 及分册 `07A~07F` —— 5 阶段 × 15 步骤详细操作（命令/验收/回退以此为准）
