...
> 文档位置：`D:\Code_Projects\Project\cloud-sms\AI代码编写规范.md`（根目录，随项目走）
...
5. **注释**：新增类/公开方法必须有中文 Javadoc 风格注释（说明"为什么"）；改动他人代码用 `//` 标注原因。
   - 🚫 **注释里禁止出现"文档引用"与"进度编号"**（2026-09-17 用户要求）：不得写书名号文档名、章节号（`§3.4`）、文件路径、`阶段N`、`第N步`、`详见 XXX` 之类。
   - 理由：注释应自包含地说明**技术原因**；实现者/后来者读代码时不该被"去翻某文档某节"打断，且文档改名/改版后注释立即失效。
   - ✅ 正确：`/** 刻意设计为可变类：链路中各过滤器依次写入不同字段，record 每步都要 copy；且 MQ 反序列化需无参构造 */`
   - ❌ 错误：`/** 详见《AI代码编写规范》§3.4 明确禁止使用新特性的场景 */`、`/** 该字段待阶段三评估删除 */`
...
5. **提交纪律**：一个模块一个 commit，message 格式 `类型: 描述`（feat/docs/chore/refactor，**不使用括号**）；不混入无关文件。
   ...
   - 🛑 **任何 commit 都必须先经用户明确同意**（2026-09-17 用户要求，**优先级高于"一步一 commit"**）：AI 完成一步后，只汇报「改了什么 + 编译结果 + 建议的 commit message」，**等用户说"提交"才执行 `git commit`**；未经同意不得 commit，也不得 push。
     - 文件照常写入工作区（写代码、改文档都不需要先问），**被拦住的只是 `git commit` / `git push` 这两个动作**。
     - `git add` 也一并不做，避免留下"已暂存但未提交"的中间态。
